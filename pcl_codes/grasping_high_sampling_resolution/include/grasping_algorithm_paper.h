...
void object_pose_approximation_and_sampling(){
  ...
  object_centroid_in_arm_hand_frame.get(object_centroid_point_in_arm_hand_frame);
	//scene_cloud_viewer->addSphere(object_centroid_point_in_arm_hand_frame, 0.02, 0, 250, 250, "object_centroid_sphere", 0);
}
...
void constructing_special_ellipsoids(void){
  ...
  object_plane_x = 0.2;       object_plane_y = 0.05;         object_plane_z = 0.2;
...
void load_allegro_right_hand_workspace_spheres( pcl::PointCloud<pcl::PointXYZ>::Ptr& gripper_augmented_workspace_xyz, 
  ...
  *gripper_augmented_workspace_xyz += *pinky_workspace_convex_xyz;
  ...
  Eigen::Vector4f dummy_3;
	dummy_3 << gripper_workspace_centroid_point_in_gripper_frame_xyz.x, gripper_workspace_centroid_point_in_gripper_frame_xyz.y, gripper_workspace_centroid_point_in_gripper_frame_xyz.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	pcl::PointXYZ dummy_3_point;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	//scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 255, 255, 0, "gripper_workspace_centroid_sphere", 0);
  
	/*
	// thumb_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> thumb_workspace_centroid;
	pcl::PointXYZ thumb_workspace_centroid_point;
	for(unsigned int i=0;i<thumb_workspace_convex_xyz->points.size();i++)
		thumb_workspace_centroid.add( thumb_workspace_convex_xyz->points[i] );
	thumb_workspace_centroid.get(thumb_workspace_centroid_point);
	dummy_3 << thumb_workspace_centroid_point.x, thumb_workspace_centroid_point.y, thumb_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 255, 0, 0, "thumb_workspace_centroid_sphere", 0);
	
	
	// index_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> index_workspace_centroid;
	pcl::PointXYZ index_workspace_centroid_point;
	for(unsigned int i=0;i<index_workspace_convex_xyz->points.size();i++)
		index_workspace_centroid.add( index_workspace_convex_xyz->points[i] );
	index_workspace_centroid.get(index_workspace_centroid_point);
	dummy_3 << index_workspace_centroid_point.x, index_workspace_centroid_point.y, index_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 0, 255, 0, "index_workspace_centroid_sphere", 0);
	
	
	// middle_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> middle_workspace_centroid;
	pcl::PointXYZ middle_workspace_centroid_point;
	for(unsigned int i=0;i<middle_workspace_convex_xyz->points.size();i++)
		middle_workspace_centroid.add( middle_workspace_convex_xyz->points[i] );
	middle_workspace_centroid.get(middle_workspace_centroid_point);
	dummy_3 << middle_workspace_centroid_point.x, middle_workspace_centroid_point.y, middle_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 0, 0, 255, "middle_workspace_centroid_sphere", 0);
	
	
	// pinky_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> pinky_workspace_centroid;
	pcl::PointXYZ pinky_workspace_centroid_point;
	for(unsigned int i=0;i<pinky_workspace_convex_xyz->points.size();i++)
		pinky_workspace_centroid.add( pinky_workspace_convex_xyz->points[i] );
	pinky_workspace_centroid.get(pinky_workspace_centroid_point);
	dummy_3 << pinky_workspace_centroid_point.x, pinky_workspace_centroid_point.y, pinky_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 0, 0, 0, "pinky_workspace_centroid_sphere", 0);
  */
...
void load_franka_gripper_workspace_spheres( pcl::PointCloud<pcl::PointXYZ>::Ptr& gripper_augmented_workspace_xyz, 
  ...
  Eigen::Vector4f dummy_3;
	...
	/*
	// right_finger_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> right_finger_workspace_centroid;
	pcl::PointXYZ right_finger_workspace_centroid_point;
	for(unsigned int i=0;i<right_finger_workspace_convex_xyz->points.size();i++)
		right_finger_workspace_centroid.add( right_finger_workspace_convex_xyz->points[i] );
	right_finger_workspace_centroid.get(right_finger_workspace_centroid_point);
	dummy_3 << right_finger_workspace_centroid_point.x, right_finger_workspace_centroid_point.y, right_finger_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 255, 0, 0, "right_finger_workspace_centroid_sphere", 0);
	
	
	// left_finger_workspace_centroid_point
	pcl::CentroidPoint<pcl::PointXYZ> left_finger_workspace_centroid;
	pcl::PointXYZ left_finger_workspace_centroid_point;
	for(unsigned int i=0;i<left_finger_workspace_convex_xyz->points.size();i++)
		left_finger_workspace_centroid.add( left_finger_workspace_convex_xyz->points[i] );
	left_finger_workspace_centroid.get(left_finger_workspace_centroid_point);
	dummy_3 << left_finger_workspace_centroid_point.x, left_finger_workspace_centroid_point.y, left_finger_workspace_centroid_point.z, 1;
	dummy_3 = gripper_wrt_arm_hand_frame_transform*dummy_3;
	dummy_3_point.x = dummy_3(0);
	dummy_3_point.y = dummy_3(1);
	dummy_3_point.z = dummy_3(2);
	scene_cloud_viewer->addSphere(dummy_3_point, 0.02, 0, 255, 0, "left_finger_workspace_centroid_sphere", 0);
	*/
    ...
    for(unsigned int j=0; j<orientation_samples_in_x; j++){				// sampling orientations about one axis (say x-axis)
			...
			for(unsigned int m=0; m<orientation_samples_in_y; m++){			// sampling orientations about one axis (say y-axis)
				...
				for(unsigned int n=0; n<orientation_samples_in_z; n++){		// sampling orientations about one axis (say z-axis)
					...
					if(gripper_collides_with_object or gripper_collides_with_object_plane){
						pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> cyan_color          (gripper_cloud_transformed_in_arm_hand_frame_xyz, 255, 0, 0);
					...
					if(gripper_model == "allegro_right_hand"){
						...
						pcl::transformPointCloud(*gripper_support_point_cloud_in_arm_hand_frame_1, *gripper_support_point_cloud_transformed_in_arm_hand_frame_1, gripper_transform);
						...
						pcl::transformPointCloud(*gripper_support_point_cloud_in_arm_hand_frame_2, *gripper_support_point_cloud_transformed_in_arm_hand_frame_2, gripper_transform);
						...
						pcl::transformPointCloud(*gripper_support_point_cloud_in_arm_hand_frame_3, *gripper_support_point_cloud_transformed_in_arm_hand_frame_3, gripper_transform);
						...
						pcl::transformPointCloud(*gripper_support_point_cloud_in_arm_hand_frame_4, *gripper_support_point_cloud_transformed_in_arm_hand_frame_4, gripper_transform);
					...
					if(gripper_model == "allegro_right_hand")
						save_file_name = "../icra2020_sim_video/allegro_vid/"+std::to_string(i)+"_"+std::to_string(j)+"_"+std::to_string(m)+"_"+std::to_string(n)+".png";
					else if(gripper_model == "franka_gripper")
						save_file_name = "../icra2020_sim_video/franka_vid/"+std::to_string(i)+"_"+std::to_string(j)+"_"+std::to_string(m)+"_"+std::to_string(n)+".png";
...
void visualize_workspace_spheres_and_object_points_for_best_gripper_pose(void){
  ...
  if(gripper_model == "allegro_right_hand"){
    ...
    */
		...
		// thumb
    scene_cloud_viewer->addPointCloud(thumb_workspace_spheres_best, red_color, "thumb workspace spheres");
    for(unsigned int j=0; j<thumb_workspace_convex_parameter->size(); j++){
      parameter_vector << thumb_workspace_convex_parameter->points[j].x, thumb_workspace_convex_parameter->points[j].y, thumb_workspace_convex_parameter->points[j].z;
      offset_vector    << thumb_workspace_convex_offset->points[j].x, thumb_workspace_convex_offset->points[j].y, thumb_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 20, 2 );
      *thumb_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*thumb_workspace_spheres_best, *thumb_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
		scene_cloud_viewer->updatePointCloud(thumb_workspace_spheres_best, red_color, "thumb workspace spheres");
    */
    ...
    */
		...
		// index
    scene_cloud_viewer->addPointCloud(index_workspace_spheres_best, green_color, "index workspace spheres");
    for(unsigned int j=0; j<index_workspace_convex_parameter->size(); j++){
      parameter_vector << index_workspace_convex_parameter->points[j].x, index_workspace_convex_parameter->points[j].y, index_workspace_convex_parameter->points[j].z;
      offset_vector    << index_workspace_convex_offset->points[j].x, index_workspace_convex_offset->points[j].y, index_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 20, 2 );
      *index_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*index_workspace_spheres_best, *index_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
		scene_cloud_viewer->updatePointCloud(index_workspace_spheres_best, green_color, "index workspace spheres");
		*/
		/*
    // middle
    ...
    */
		...
		// middle
    scene_cloud_viewer->addPointCloud(middle_workspace_spheres_best, blue_color, "middle workspace spheres");
    for(unsigned int j=0; j<middle_workspace_convex_parameter->size(); j++){
      parameter_vector << middle_workspace_convex_parameter->points[j].x, middle_workspace_convex_parameter->points[j].y, middle_workspace_convex_parameter->points[j].z;
      offset_vector    << middle_workspace_convex_offset->points[j].x, middle_workspace_convex_offset->points[j].y, middle_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 20, 2 );
      *middle_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*middle_workspace_spheres_best, *middle_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
		scene_cloud_viewer->updatePointCloud(middle_workspace_spheres_best, blue_color, "middle workspace spheres");
		*/
		/*
    // pinky
    ...
    scene_cloud_viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 15, "pinky workspace points");
		*/
		...
		// pinky
    scene_cloud_viewer->addPointCloud(pinky_workspace_spheres_best, grey_color, "pinky workspace spheres");
    for(unsigned int j=0; j<pinky_workspace_convex_parameter->size(); j++){
      parameter_vector << pinky_workspace_convex_parameter->points[j].x, pinky_workspace_convex_parameter->points[j].y, pinky_workspace_convex_parameter->points[j].z;
      offset_vector    << pinky_workspace_convex_offset->points[j].x, pinky_workspace_convex_offset->points[j].y, pinky_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 20, 2 );
      *pinky_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*pinky_workspace_spheres_best, *pinky_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
		scene_cloud_viewer->updatePointCloud(pinky_workspace_spheres_best, grey_color, "pinky workspace spheres");
		*/
  }
  else if(gripper_model == "franka_gripper"){
    ...
    scene_cloud_viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 20, "right_finger workspace points");
		*/
		...
		// right_finger
    scene_cloud_viewer->addPointCloud(right_finger_workspace_spheres_best, red_color_again, "right_finger workspace spheres");
    for(unsigned int j=0; j<right_finger_workspace_convex_parameter->size(); j++){
      parameter_vector << right_finger_workspace_convex_parameter->points[j].x, right_finger_workspace_convex_parameter->points[j].y, right_finger_workspace_convex_parameter->points[j].z;
      offset_vector    << right_finger_workspace_convex_offset->points[j].x, right_finger_workspace_convex_offset->points[j].y, right_finger_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 50, 2 );
      *right_finger_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*right_finger_workspace_spheres_best, *right_finger_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
    scene_cloud_viewer->updatePointCloud(right_finger_workspace_spheres_best, red_color_again, "right_finger workspace spheres");
    */
		/*
    // left_finger
    ...
    scene_cloud_viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 20, "left_finger workspace points");
		*/
		...
		// left_finger
    scene_cloud_viewer->addPointCloud(left_finger_workspace_spheres_best, green_color_again, "left_finger workspace spheres");
    for(unsigned int j=0; j<left_finger_workspace_convex_parameter->size(); j++){
      parameter_vector << left_finger_workspace_convex_parameter->points[j].x, left_finger_workspace_convex_parameter->points[j].y, left_finger_workspace_convex_parameter->points[j].z;
      offset_vector    << left_finger_workspace_convex_offset->points[j].x, left_finger_workspace_convex_offset->points[j].y, left_finger_workspace_convex_offset->points[j].z;
      construct_special_ellipsoid_point_cloud( dummy_cloud_xyz, parameter_vector, offset_vector, 50, 2 );
      *left_finger_workspace_spheres_best += *dummy_cloud_xyz;}
    pcl::transformPointCloud(*left_finger_workspace_spheres_best, *left_finger_workspace_spheres_best, gripper_wrt_arm_hand_frame_transform);
    scene_cloud_viewer->updatePointCloud(left_finger_workspace_spheres_best, green_color_again, "left_finger workspace spheres");
		*/
  }
}

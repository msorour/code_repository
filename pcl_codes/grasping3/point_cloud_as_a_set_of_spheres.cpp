// reset && cmake .. && make && ./point_cloud_as_a_set_of_spheres ../finger_workspace_pcd/middle_workspace.pcd false
...
int main(int argc, char **argv){
  ...
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> red  (finger_workspace_spheres_visualization_xyz, 255, 0, 0);
  viewer->addPointCloud<pcl::PointXYZ>(finger_workspace_spheres_visualization_xyz, red, "finger spheres");
  ...
  double radius_of_smallest_sphere = 0.015;
  double radius_of_largest_sphere  = 0.035;
  double sphere_radius_increment   = 0.001;
  int sphere_point_cloud_samples   = 50;
  int desired_number_of_spheres    = 15;
  int iterations = 5;
  std::vector<double> sphere_radius_filtered;
  point_cloud_as_set_of_spheres( desired_number_of_spheres, finger_workspace_cloud_xyz, radius_of_smallest_sphere, radius_of_largest_sphere, sphere_radius_increment, iterations, sphere_point_cloud_samples, finger_workspace_spheres_filtered_xyz, sphere_radius_filtered, finger_workspace_spheres_visualization_xyz, far_point_in_pos_direction_4d, far_point_in_neg_direction_4d );
  ...
  viewer->updatePointCloud(finger_workspace_spheres_visualization_xyz, red, "finger spheres");
  ...
  std::string file_name = workspace_cloud_file;
  file_name.erase(file_name.end()-4, file_name.end());
  file_name = file_name + std::string("_spheres.txt");

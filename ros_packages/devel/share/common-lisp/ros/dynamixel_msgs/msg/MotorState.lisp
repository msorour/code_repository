...
(cl:defmethod roslisp-msg-protocol:serialize ((msg <MotorState>) ostream)
  ...
  (cl:let* ((signed (cl:slot-value msg 'goal)) (unsigned (cl:if (cl:< signed 0) (cl:+ signed 18446744073709551616) signed)))
    (cl:write-byte (cl:ldb (cl:byte 8 0) unsigned) ostream)
    (cl:write-byte (cl:ldb (cl:byte 8 8) unsigned) ostream)
    (cl:write-byte (cl:ldb (cl:byte 8 16) unsigned) ostream)
    (cl:write-byte (cl:ldb (cl:byte 8 24) unsigned) ostream)
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <MotorState>) istream)
  "Deserializes a message object of type '<MotorState>"
    ...
    (cl:let ((unsigned 0))
      ...
      (cl:setf (cl:slot-value msg 'goal) (cl:if (cl:< unsigned 9223372036854775808) unsigned (cl:- unsigned 18446744073709551616))))
    (cl:let ((unsigned 0))
      (cl:setf (cl:ldb (cl:byte 8 0) unsigned) (cl:read-byte istream))
      (cl:setf (cl:ldb (cl:byte 8 8) unsigned) (cl:read-byte istream))
      (cl:setf (cl:ldb (cl:byte 8 16) unsigned) (cl:read-byte istream))
...
(cl:defmethod roslisp-msg-protocol:md5sum ((type (cl:eql '<MotorState>)))
  ...
  "2184111e0e6b6ad4961bf5f12a5055e3")
(cl:defmethod roslisp-msg-protocol:md5sum ((type (cl:eql 'MotorState)))
  "Returns md5sum for a message object of type 'MotorState"
  "2184111e0e6b6ad4961bf5f12a5055e3")
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<MotorState>)))
  ...
  (cl:format cl:nil "float64 timestamp   # motor state is at this time~%int32 id            # motor id~%int64 goal          # commanded position (in encoder units)~%int64 position      # current position (in encoder units)~%int64 error         # difference between current and goal positions~%int64 speed         # current speed (0.111 rpm per unit)~%float64 load        # current load - ratio of applied torque over maximum torque~%float64 voltage     # current voltage (V)~%int32 temperature   # current temperature (degrees Celsius)~%bool moving         # whether the motor is currently in motion~%#TODO check int64 vs int32~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'MotorState)))
  "Returns full string definition for message of type 'MotorState"
  (cl:format cl:nil "float64 timestamp   # motor state is at this time~%int32 id            # motor id~%int64 goal          # commanded position (in encoder units)~%int64 position      # current position (in encoder units)~%int64 error         # difference between current and goal positions~%int64 speed         # current speed (0.111 rpm per unit)~%float64 load        # current load - ratio of applied torque over maximum torque~%float64 voltage     # current voltage (V)~%int32 temperature   # current temperature (degrees Celsius)~%bool moving         # whether the motor is currently in motion~%#TODO check int64 vs int32~%~%"))

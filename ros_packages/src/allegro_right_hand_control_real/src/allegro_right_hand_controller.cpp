...
#include "../include/allegro_hand_interface.h"
...
int main(int argc, char **argv){
  ...
  //cout << "index_DGM_iterative  = " << endl << general_transformation_matrix(4,0,r_index,d_index,alpha_index,theta_index+index_joint_position) << endl;
    ...
    VectorXd q_curr(16), q_desired(16);
    Vector4d q_index_curr, q_middle_curr, q_pinky_curr, q_thumb_curr;
    q_curr = GetJointPosition();
    for(int k=0; k<4; k++)
		  q_index_curr(k)  = q_curr(k);
    for(int k=0; k<4; k++)
		  q_middle_curr(k) = q_curr(k+4);
		for(int k=0; k<4; k++)
		  q_pinky_curr(k) = q_curr(k+8);
		for(int k=0; k<4; k++)
		  q_thumb_curr(k) = q_curr(k+12);
		
    cout << "index joint position:  " << q_index_curr.transpose()  << endl;
    cout << "middle joint position: " << q_middle_curr.transpose() << endl;
    cout << "pinky joint position:  " << q_pinky_curr.transpose()  << endl;
    cout << "thumb joint position:  " << q_thumb_curr.transpose()  << endl;
    
    q_desired << 0.05, -0.1, 0.8, 0.8, 0.04, -0.17, 0.8, 0.8, 0.05, -0.13, 0.91, 0.8, 0.8, 0.4, 0.06, 0.8;
    SetDesiredJointPosition(q_desired);

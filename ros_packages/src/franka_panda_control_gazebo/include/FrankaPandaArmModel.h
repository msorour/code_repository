...
Matrix3d skew_symm_matrix(Vector3d vec);
...
MatrixXd arm_geometric_jacobian_matrix(VectorXd joint_state){
	...
	// Jacobian matrix for frame 7, Projection frame 7, intermediate frame 7
	// Column#1
	...
	*/
	...
	Vector3d r_7E, r_E7;
	Matrix3d R_E7, r_7E_hat, product;
	Matrix3d R_7E  = Matrix3d::Identity();
	Matrix3d zero3 = Matrix3d::Zero();
	MatrixXd screw_TM_E7(6,6);
	r_7E << 0, 0, db;
	R_E7 = R_7E.transpose();
...
MatrixXd geometric_jacobian_derivative(MatrixXd jacobian_past(6,7), double time_now, double time_past){
	MatrixXd jacobian_now(6,7);
	MatrixXd jacobian_derivative(6,7);
...
*/
/*
MatrixXd geometric_jacobian_derivative(VectorXd joint_state){
	double t1, t2, t3, t4, t5, t6, t7;
	t1 = joint_state(0);
	t2 = joint_state(1);
	t3 = joint_state(2);
...
MatrixXd arm_inertia_matrix(VectorXd joint_state){
	double t1, t2, t3, t4, t5, t6, t7;
	t1 = joint_state(0);
	t2 = joint_state(1);
	t3 = joint_state(2);
...
VectorXd arm_viscous_friction_torque(VectorXd joint_velocity){
	MatrixXd viscous_friction_coefficient_matrix(7,7);
...
VectorXd arm_static_friction_torque(VectorXd joint_velocity){
	VectorXd joint_velocity_sign(joint_velocity.size());
	...
	MatrixXd static_friction_coefficient_matrix(7,7);
...
VectorXd arm_coriolis_centrifugal_torque(VectorXd joint_state, VectorXd joint_velocity){
...
VectorXd transfer_matrix_to_rose_rpy(Matrix4d TM)
{
	VectorXd POSE(6);
...
MatrixXd Pinv_damped( MatrixXd J , double damping )    // Moore-Penrose Pseudo-inverse computing function
{
  int r = J.rows();
  int c = J.cols();
  MatrixXd Jpinv( c , r );

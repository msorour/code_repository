...
int main(int argc, char **argv){
  ...
  ros::Rate loop_rate(500);
	...
	pose_rpy_bE_desired << -0.3, 0, 0.6, pi, 0, 0;
	//velocity_bE_desired << 0, 0, 0, 0, 0.1, 0.1;
  ...
  //while (ros::ok()){
  	...
  	//analytic_jacobian = Pinv_damped( geometric_to_analytic_jacobian_rpy(pose_rpy_bE) , 0.001 )*geometric_jacobian;
  	...
  	*/
  	...
  	velocity_error = pose_error;
  	joint_velocity_error = Pinv_damped(analytic_jacobian, 0.001)*velocity_error;

...
Eigen::MatrixXd thumb_position_jacobian_past(3,4), index_position_jacobian_past(3,4), middle_position_jacobian_past(3,4), pinky_position_jacobian_past(3,4);
Eigen::MatrixXd thumb_position_jacobian_dot(3,4), index_position_jacobian_dot(3,4), middle_position_jacobian_dot(3,4), pinky_position_jacobian_dot(3,4);

Eigen::Vector4d thumb_joint_position_desired,  thumb_joint_velocity_desired,  thumb_joint_torque_command,  thumb_joint_velocity_command,  thumb_joint_position,  thumb_joint_position_past,  thumb_joint_velocity;
Eigen::Vector4d index_joint_position_desired,  index_joint_velocity_desired,  index_joint_torque_command,  index_joint_velocity_command,  index_joint_position,  index_joint_position_past,  index_joint_velocity;
Eigen::Vector4d middle_joint_position_desired, middle_joint_velocity_desired, middle_joint_torque_command, middle_joint_velocity_command, middle_joint_position, middle_joint_position_past, middle_joint_velocity;
Eigen::Vector4d pinky_joint_position_desired,  pinky_joint_velocity_desired,  pinky_joint_torque_command,  pinky_joint_velocity_command,  pinky_joint_position,  pinky_joint_position_past,  pinky_joint_velocity;
...
Eigen::Vector3d velocity_Ptt, velocity_Ptt_desired, velocity_Ptt_init, velocity_Ptt_traj, velocity_Ptt_error;
Eigen::Vector3d velocity_Pit, velocity_Pit_desired, velocity_Pit_init, velocity_Pit_traj, velocity_Pit_error;
Eigen::Vector3d velocity_Pmt, velocity_Pmt_desired, velocity_Pmt_init, velocity_Pmt_traj, velocity_Pmt_error;
Eigen::Vector3d velocity_Ppt, velocity_Ppt_desired, velocity_Ppt_init, velocity_Ppt_traj, velocity_Ppt_error;
...
// logs
std::ofstream thumb_tip_position_trajectory_log;
std::ofstream thumb_tip_position_measured_log;
std::ofstream thumb_tip_position_error_log;
std::ofstream thumb_tip_velocity_trajectory_log;
std::ofstream thumb_tip_velocity_measured_log;
std::ofstream thumb_tip_velocity_error_log;

std::ofstream thumb_joint_position_trajectory_log;
std::ofstream thumb_joint_position_measured_log;
std::ofstream thumb_joint_position_error_log;
std::ofstream thumb_joint_velocity_trajectory_log;
std::ofstream thumb_joint_velocity_measured_log;
std::ofstream thumb_joint_velocity_error_log;

std::ofstream sample_time_log;
...
// logs
void log_data(void){
	thumb_tip_position_trajectory_log << time_now << " " << position_Ptt_traj.transpose()  <<  std::endl;
	thumb_tip_position_measured_log   << time_now << " " << position_Ptt.transpose()       <<  std::endl;
	thumb_tip_position_error_log      << time_now << " " << position_Ptt_error.transpose() <<  std::endl;
	thumb_tip_velocity_trajectory_log << time_now << " " << velocity_Ptt_traj.transpose()  <<  std::endl;
	thumb_tip_velocity_measured_log   << time_now << " " << velocity_Ptt.transpose()       <<  std::endl;
	thumb_tip_velocity_error_log      << time_now << " " << velocity_Ptt_error.transpose() <<  std::endl;
	
	//thumb_joint_position_trajectory_log << time_now << " " << thumb_joint_position.transpose()  <<  std::endl;
	thumb_joint_position_measured_log   << time_now << " " << thumb_joint_position.transpose()       <<  std::endl;
	//thumb_joint_position_error_log      << time_now << " " << position_Ptt_error.transpose() <<  std::endl;
	//thumb_joint_velocity_trajectory_log << time_now << " " << velocity_Ptt_traj.transpose()  <<  std::endl;
	thumb_joint_velocity_measured_log   << time_now << " " << thumb_joint_velocity.transpose()       <<  std::endl;
	//thumb_joint_velocity_error_log      << time_now << " " << velocity_Ptt_error.transpose() <<  std::endl;
	
	sample_time_log                   << time_now << " " << sample_time <<  std::endl;
}

void open_logs(void){
	thumb_tip_position_trajectory_log.open("/home/work/code_repository/allegro_hand_control/logs/thumb_tip_position_trajectory_log.txt");
	thumb_tip_position_measured_log.open(  "/home/work/code_repository/allegro_hand_control/logs/thumb_tip_position_measured_log.txt");
	thumb_tip_position_error_log.open(     "/home/work/code_repository/allegro_hand_control/logs/thumb_tip_position_error_log.txt");
	thumb_tip_velocity_trajectory_log.open("/home/work/code_repository/allegro_hand_control/logs/thumb_tip_velocity_trajectory_log.txt");
	thumb_tip_velocity_measured_log.open(  "/home/work/code_repository/allegro_hand_control/logs/thumb_tip_velocity_measured_log.txt");
	thumb_tip_velocity_error_log.open(     "/home/work/code_repository/allegro_hand_control/logs/thumb_tip_velocity_error_log.txt");
	
	//thumb_joint_position_trajectory_log.open("/home/work/code_repository/allegro_hand_control/logs/thumb_joint_position_trajectory_log.txt");
	thumb_joint_position_measured_log.open(  "/home/work/code_repository/allegro_hand_control/logs/thumb_joint_position_measured_log.txt");
	//thumb_joint_position_error_log.open(     "/home/work/code_repository/allegro_hand_control/logs/thumb_joint_position_error_log.txt");
	//thumb_joint_velocity_trajectory_log.open("/home/work/code_repository/allegro_hand_control/logs/thumb_joint_velocity_trajectory_log.txt");
	thumb_joint_velocity_measured_log.open(  "/home/work/code_repository/allegro_hand_control/logs/thumb_joint_velocity_measured_log.txt");
	//thumb_joint_velocity_error_log.open(     "/home/work/code_repository/allegro_hand_control/logs/thumb_joint_velocity_error_log.txt");
	
	sample_time_log.open(     "/home/work/code_repository/allegro_hand_control/logs/sample_time_log.txt");
}
void close_logs(void){
	thumb_tip_position_trajectory_log.close();
	thumb_tip_position_measured_log.close();
	thumb_tip_position_error_log.close();
	thumb_tip_velocity_trajectory_log.close();
	thumb_tip_velocity_measured_log.close();
	thumb_tip_velocity_error_log.close();
	
	//thumb_joint_position_trajectory_log.close();
	thumb_joint_position_measured_log.close();
	//thumb_joint_position_error_log.close();
	//thumb_joint_velocity_trajectory_log.close();
	thumb_joint_velocity_measured_log.close();
	//thumb_joint_velocity_error_log.close();
	
	sample_time_log.close();
}
...
int main(int argc, TCHAR* argv[]){
  ...
  if(CreateBHandAlgorithm() && OpenCAN()){
    ...
    index_joint_position_past  = index_joint_position;
    middle_joint_position_past = middle_joint_position;
    pinky_joint_position_past  = pinky_joint_position;
    thumb_joint_position_past  = thumb_joint_position;
    ...
    while( ros::ok() and (time_now-start_time) < trajectory_duration ){
      ...
      index_joint_velocity  = (index_joint_position -index_joint_position_past) /sample_time;
      middle_joint_velocity = (middle_joint_position-middle_joint_position_past)/sample_time;
      pinky_joint_velocity  = (pinky_joint_position -pinky_joint_position_past) /sample_time;
      thumb_joint_velocity  = (thumb_joint_position -thumb_joint_position_past) /sample_time;
      ...
      thumb_position_jacobian_dot  = (thumb_position_jacobian -thumb_position_jacobian_past) /sample_time;
      index_position_jacobian_dot  = (index_position_jacobian -index_position_jacobian_past) /sample_time;
      middle_position_jacobian_dot = (middle_position_jacobian-middle_position_jacobian_past)/sample_time;
      pinky_position_jacobian_dot  = (pinky_position_jacobian -pinky_position_jacobian_past) /sample_time;
      ...
      position_Ptt << pose_rpy_Ptt(0), pose_rpy_Ptt(1), pose_rpy_Ptt(2);
	    ...
	    velocity_Ptt = thumb_position_jacobian*thumb_joint_velocity;
	    velocity_Pit = index_position_jacobian*index_joint_velocity;
	    velocity_Pmt = middle_position_jacobian*middle_joint_velocity;
	    velocity_Ppt = pinky_position_jacobian*pinky_joint_velocity;
      ...
      time_now = (double)(now)/CLOCKS_PER_SEC;
	    ...
	    velocity_Ptt_traj << x_traj(1), y_traj(1), z_traj(1);
	    ...
	    */
	    ...
	    velocity_Ptt_error = velocity_Ptt_traj - velocity_Ptt;
	    velocity_Pit_error = velocity_Pit_desired - velocity_Pit;
	    velocity_Pmt_error = velocity_Pmt_desired - velocity_Pmt;
	    velocity_Ppt_error = velocity_Ppt_desired - velocity_Ppt;
	    
	    velocity_Ptt_desired_3d = 0.3*velocity_Ptt_error + 3.7*position_Ptt_error;
	    ...
	    */
	    finger_null_space_projector = I4 - Pinv_damped(thumb_position_jacobian, 0.001)*thumb_position_jacobian;
	    finger_task_gradient = avoid_joint_limit_task_gradient(thumb_joint_position, thumb_joint_safe_mean, thumb_joint_safe_range);
	    thumb_joint_velocity_command  = Pinv_damped(thumb_position_jacobian, 0.001)*(velocity_Ptt_desired_3d-thumb_position_jacobian_dot*thumb_joint_velocity);// + lambda*finger_null_space_projector*finger_task_gradient;
      ...
      index_position_jacobian_past = index_position_jacobian;
      middle_position_jacobian_past = middle_position_jacobian;
      pinky_position_jacobian_past = pinky_position_jacobian;
      thumb_position_jacobian_past = thumb_position_jacobian;
